...
const PhaserGame = ({ sessionId, counter, onGameEnd }) => {
  ...
  useEffect(() => {
    const handleVisibilityChange = () => {
      if (document.hidden) {
        // Store the time when the window is hidden
        setPauseTime(new Date());
      } else {
        // Calculate the total pause duration and adjust the counter
        if (pauseTime) {
          const now = new Date();
          const pauseDuration = Math.floor((now - pauseTime) / 1000);
          setCurrentCounter(prevCounter => Math.max(prevCounter - pauseDuration, 0));
          setPauseTime(null);
        }
      }
    };
  ...
  useEffect(() => {
    if (currentCounter <= 0 && sessionActive) {
      handleGameEnd();
    }
  }, [currentCounter, sessionActive]);
  ...
  const startGame = () => {
    ...
    if (!gameRef.current) {
      ...
      function update() {
        if (this.counterText) {
          this.counterText.setText(`Counter: ${currentCounter}`);
        }
      }
    } else {
      const scene = gameRef.current.scene.getScene('mainScene');
